...
import { z } from "zod";
import { SHOP_ROLE_SCHEMA } from "@/lib/roles";

const roleSchema = SHOP_ROLE_SCHEMA;
...
export async function updateTeamMemberRole(formData: FormData) {
  ...
  if (target.role === "OWNER" && parsedRole.data !== "OWNER") {
    ...
    if (ownerCount <= 1) {
      return { error: "Debe haber al menos un administrador en la agencia" };
    }
  }
...
export async function deleteTeamMember(userId: string) {
  ...
  if (target.role === "OWNER") {
    ...
    if (ownerCount <= 1) {
      return { error: "No puedes eliminar al único administrador de la agencia" };
    }
  }
