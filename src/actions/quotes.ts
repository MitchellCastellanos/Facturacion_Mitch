...
import {
  billingProfileToEmailConfig,
  billingProfileToIssuer,
  formatWorkContext,
} from "@/lib/billing-profile-document";
import { resolveBillingProfile } from "@/lib/billing-profile-resolve";
...
export async function createQuote(formData: QuoteFormData) {
  ...
  const profile = await resolveBillingProfile(shopId, billingProfileId);
  ...
  const quote = await db.$transaction(async (tx) => {
    const quoteNumber = await allocateNextQuoteNumber(tx, shopId, profile.invoicePrefix);
...
export async function sendQuoteByEmail(id: string, formData?: FormData) {
  ...
  const vehicleDescription = formatWorkContext(quote);

  try {
    await sendQuoteEmail({
      shop: billingProfileToEmailConfig(quote.billingProfile, quote.shop),
      to: clientEmail,
      pdfBuffer,
      ...
      shopPhone: issuer.phone,
      shopAddress: issuer.address,
      shopLogoUrl: issuer.logoUrl,
...
export async function convertQuoteToInvoice(id: string) {
  ...
  const profile = quote.billingProfileId
    ? await db.billingProfile.findFirst({ where: { id: quote.billingProfileId, shopId } })
    : await resolveBillingProfile(shopId, null);
...
export async function getQuoteFormData() {
  ...
  const [clients, shop, billingProfiles] = await Promise.all([
    db.client.findMany({
      where: { shopId },
    ...
    db.billingProfile.findMany({
      where: { shopId, isActive: true },
      orderBy: [{ isDefault: "desc" }, { legalName: "asc" }],
    }),
  ]);

  return { clients, shop, billingProfiles };
}
