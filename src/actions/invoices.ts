  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      invoice = await db.$transaction(async (tx) => {
        const invoiceNumber = await allocateNextInvoiceNumber(tx, shopId, profile.invoicePrefix);
...
export async function sendInvoiceByEmail(id: string, formData?: FormData) {
  ...
  const vehicleDescription = formatWorkContext(invoice);
  ...
  try {
    await sendInvoiceEmail({
      shop: billingProfileToEmailConfig(invoice.billingProfile, invoice.shop),
      to: clientEmail,
