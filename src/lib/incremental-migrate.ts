...
export const SCHEMA_VERSION = "20260608-agency-cleanup-v3";
...
export const INCREMENTAL_MIGRATE_STATEMENTS = [
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS "BillingProfile_shopId_code_key" ON mecanico."BillingProfile"("shopId", "code")`,
  `DO $$ BEGIN
...
export async function ensureMemberRoleEnum() {
  try {
    await db.$executeRawUnsafe(`ALTER TYPE mecanico."Role" ADD VALUE IF NOT EXISTS 'MEMBER'`);
  } catch (err) {
...
async function migrateMechanicToMember() {
  await db.$executeRawUnsafe(
    `UPDATE mecanico."User" SET role = 'MEMBER' WHERE role = 'MECHANIC'`
  );
}

const LINE_ITEM_TYPE_VALUES = [
  "DEVELOPMENT",
  "DESIGN",
  "HOSTING",
  "MAINTENANCE",
...
export async function ensureLineItemTypeEnumValues() {
  for (const value of LINE_ITEM_TYPE_VALUES) {
    try {
      await db.$executeRawUnsafe(
        `ALTER TYPE mecanico."LineItemType" ADD VALUE IF NOT EXISTS '${value}'`
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      if (!msg.includes("already exists") && !msg.includes("duplicate")) {
...
export async function runIncrementalMigrate() {
  ...
  await ensureLineItemTypeEnumValues();
  await ensureMemberRoleEnum();
  for (const statement of INCREMENTAL_MIGRATE_STATEMENTS) {
    await db.$executeRawUnsafe(statement);
  }
  await migrateMechanicToMember();
  await db.$executeRawUnsafe(
