...
export function InvoiceDocument({ invoice }: { invoice: InvoiceData }) {
  ...
  const warrantyItems = invoice.lineItems.filter(
    (item) =>
      item.warrantyTerm?.trim() &&
      ["PART", "HOSTING", "MAINTENANCE", "DEVELOPMENT"].includes(item.itemType)
  );
  const suppressTaxes = Boolean(invoice.suppressTaxes);
  const showVehicle = Boolean(invoice.vehicle);
  const showProject = Boolean(invoice.projectName?.trim());
  const showMileage =
    invoice.mileageIn != null || invoice.mileageOut != null;
        ...
        <View style={styles.body}>
          <View style={styles.cardsRow} wrap={false}>
            ...
            {showProject && (
              <View style={styles.card}>
                <View style={styles.cardHeader}>
                  <Text style={styles.cardHeaderText}>{t.project}</Text>
                </View>
                <View style={styles.cardBody}>
                  <Text style={styles.cardTitle}>{invoice.projectName}</Text>
                </View>
              </View>
            )}

            {showVehicle && invoice.vehicle && (
              <View style={styles.card}>
                <View style={styles.cardHeader}>
