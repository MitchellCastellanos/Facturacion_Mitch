...
import { LINE_ITEM_TYPE_LABELS } from "@/lib/line-item-types";
  ...
  return (
    <div className="max-w-4xl space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              ...
              {quote.billingProfile && ` · ${quote.billingProfile.legalName}`}
              {` · Idioma: ${langLabel}`}
              {quote.emailSentAt &&
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">
            {quote.projectName ? "Proyecto" : quote.vehicle ? "Vehículo" : "Referencia"}
          </p>
          {quote.projectName ? (
            <p className="font-semibold text-slate-900">{quote.projectName}</p>
          ) : quote.vehicle ? (
            <>
          ...
          {quote.lineItems.map((item) => (
            ...
            >
              ...
              <p className="hidden sm:block text-sm text-slate-500">
                {LINE_ITEM_TYPE_LABELS[item.itemType] ?? item.itemType}
              </p>
