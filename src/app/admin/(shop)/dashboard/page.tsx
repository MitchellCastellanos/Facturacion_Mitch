...
export default async function DashboardPage() {
  ...
  if (!shopId) {
    return <div className="text-center py-20 text-slate-500">Configurando tu agencia...</div>;
  }
  ...
  const [
    ...
    db.invoice.count({
      where: { shopId, status: { in: [...INVOICE_PENDING_STATUSES] } },
    }),
    db.quote.count({ where: { shopId, status: { in: ["DRAFT", "SENT"] } } }),
    db.invoice.findMany({
      where: { shopId },
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <div className="space-y-2">
            {[
              ...
              { href: "/quotes/new", label: "Nueva cotización", icon: FileSpreadsheet, bg: "bg-teal-50", color: "text-teal-600" },
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-amber-800">
              ...
              {pendingInvoices !== 1 ? "s" : ""} de cobro
            </p>
            <Link href={`${ADMIN.invoices}?status=PENDING`} className="text-xs text-amber-700 hover:underline">
              Ver facturas →
            </Link>
          </div>
        </div>
