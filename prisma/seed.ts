...
async function main() {
  ...
  const owner = await prisma.user.upsert({
    where: { email: "demo@gabansolutions.ca" },
    update: { role: "OWNER", name: "Mitchell Castellanos" },
    create: {
      shopId: shop.id,
      name: "Mitchell Castellanos",
      email: "demo@gabansolutions.ca",
      passwordHash,
      role: "OWNER",
    },
  });
  ...
  console.log(`✅ Perfiles de facturación: ${BRAND.billingProfiles.map((p) => p.legalName).join(", ")}`);
